...
若某页没有照片、官方素材、复杂插画、复杂纹理、复杂 3D、复杂光影或产品截图，则最终 `pictures` 应为 0。若第二阶段组件清单写明“无需保留图片资产”，最终也应为 0。
...
| `expected_pictures` | 预期图片数量；无复杂资产页必须为 `0` |
...
- 无照片、Logo、产品 UI、复杂插画、复杂纹理、复杂 3D、复杂光影时：`expected_pictures = 0`，`image_assets = []`，`pictures_must_be_zero = true`。
- `pictures_must_be_zero = true` 时，最终 PPTX 对应页出现任何图片即失败。
